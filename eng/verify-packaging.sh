#!/usr/bin/env bash
# =============================================================================
# verify-packaging.sh — prove the "build on Cortex as packages" thesis.
# -----------------------------------------------------------------------------
# Cortex's core promise is that a product is a *thin host that installs the
...
echo "==> Packing $ROOT/Cortex.slnx  (version $VERSION)"
dotnet pack "$(to_native "$ROOT/Cortex.slnx")" -c Release -o "$(to_native "$FEED")" -p:PackageVersion="$VERSION" >/dev/null
...
<configuration>
  <packageSources>
    ...
    <add key="cortex-local" value="$(to_native "$FEED")" />
  </packageSources>
</configuration>
EOF

# Referencing Cortex.AspNetCore pulls the whole platform graph (Core, Modules.Sdk,
...
<Project Sdk="Microsoft.NET.Sdk">
  ...
  <ItemGroup>
    ...
    <PackageReference Include="Cortex.AspNetCore" Version="$VERSION" />
  </ItemGroup>
</Project>
EOF
...
// Implementing IModule against the PACKAGE (resolved transitively via Cortex.AspNetCore)
// is the exact "build a module on Cortex" path a downstream product follows.
...
echo ""
echo "OK — Cortex packs cleanly and a fresh module project consumes the packages."
