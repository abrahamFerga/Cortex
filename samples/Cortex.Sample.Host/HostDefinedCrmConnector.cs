...
/// <summary>
/// A connector defined by the PRODUCT HOST itself — not shipped in the Cortex.Connectors package.
/// It exists to prove (and document by example) that a domain system can add its OWN connector:
/// reference Cortex.Connectors.Sdk + Cortex.Modules.Sdk, implement <see cref="IConnector"/> in your
/// own assembly, and register it with <c>builder.AddCortexConnector&lt;T&gt;()</c> exactly like a
...
public sealed class HostDefinedCrmConnector : IConnector
{
    ...
    public ConnectorManifest Manifest { get; } = new()
    {
        ...
        Description = "Illustrates a product-defined connector living in the host's own assembly, not the Cortex.Connectors package.",
