...
/// <summary>
/// Guards the SignalR chat wire contract (no server/Docker — not in the "api" collection): AddCortexRealtime
...
public sealed class RealtimeSetupTests
{
    private static JsonSerializerOptions SignalRPayloadOptions()
    {
        ...
        services.AddLogging();
        services.AddCortexRealtime(new ConfigurationBuilder().Build()); // no Redis connection → in-memory backplane
