...
public sealed class RevocationAndNotificationSettingsTests(IntegrationFixture fixture)
{
    ...
    public async Task NotificationSettings_SecretIsWriteOnly_NullKeeps_EmptyClears()
    {
        ...
        (await admin.PutAsJsonAsync("/api/admin/notification-settings",
            new { webhookUrl = "https://hooks.example/cortex", webhookSecret = "super-s3cret" }))
            .EnsureSuccessStatusCode();
        ...
        Assert.Equal("https://hooks.example/cortex", settings.GetProperty("webhookUrl").GetString());
        ...
        (await admin.PutAsJsonAsync("/api/admin/notification-settings",
            new { webhookUrl = "https://hooks.example/cortex", webhookSecret = (string?)null }))
            .EnsureSuccessStatusCode();
        ...
        (await admin.PutAsJsonAsync("/api/admin/notification-settings",
            new { webhookUrl = "https://hooks.example/cortex", webhookSecret = "" }))
            .EnsureSuccessStatusCode();
