# Cortex.Modules.Tasks — the minimal module template

The smallest useful Cortex module: an in-memory to-do list. It's the worked example behind
...
2. Reference it from your API host and call `builder.AddCortexModule<TasksModule>()`.
3. Run — RBAC, audit, token tracking, the admin dashboard, and chat all apply automatically.

It is intentionally **not** registered in `Cortex.Sample.Host`, so the demo stays focused on the three
