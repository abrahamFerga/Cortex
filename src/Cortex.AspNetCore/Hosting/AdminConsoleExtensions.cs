...
/// <summary>
/// Serves the Cortex <b>admin console</b> — the <c>@cortex/admin-ui</c> SPA — directly from the API host
/// at <c>/admin</c>. This is the platform's analogue of OpenClaw's "control UI built into the gateway":
/// every Cortex host gets a security / RBAC / users / token-usage / audit console for free, served from
...
public static class AdminConsoleExtensions
{
    ...
    public static WebApplication UseCortexAdminConsole(
        this WebApplication app,
        string? assetPath = null,
        string requestPath = DefaultRequestPath)
    {
        var logger = app.Services.GetRequiredService<ILoggerFactory>().CreateLogger("Cortex.AdminConsole");
        ...
        if (!Directory.Exists(root))
        {
            if (logger.IsEnabled(LogLevel.Information))
            {
                logger.LogInformation(
                    "Cortex admin console assets not found at {Path}; the console will not be served. " +
                    "Build @cortex/admin-ui and copy its dist/ output there to enable it.",
                    root);
            }
            return app;
        }
        ...
        if (logger.IsEnabled(LogLevel.Information))
        {
            logger.LogInformation("Cortex admin console mounted at {RequestPath} from {Path}", requestPath, root);
        }
        return app;
    }
}
