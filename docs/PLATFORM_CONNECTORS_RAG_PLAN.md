...
|---|---|---|
| **1** | RAG core: `RagCollection`/`RagChunk` + pgvector, ingestion job, hybrid+RRF query with tenant/collection filters + fail-closed recheck, `search_knowledge` tool, `MockEmbeddingGenerator`, matter-scoped collections in Legal | Immediate user value on data already in the platform; no external OAuth; **completes legal item 10** (matter ACL gate) as part of the collection gate; exercises jobs/permissions/audit seams end-to-end |
| **2** | Connector SDK + enablement: `Cortex.Connectors.Sdk`, `TenantConnector`/`ITenantConnectorStore`, admin Integrations page, `azure-blob` + `local-folder` connectors (Lane A) | Proves the contract with a service-auth connector before OAuth complexity; unblocks "attach from blob storage" |
| **3** | `cortex init` wizard (QuickStart + non-interactive), catalogs driven by manifests | Most valuable once there are real channels/connectors/RAG toggles to select; config keys will exist and be stable by then |
| **4** | `msgraph` (SharePoint/OneDrive) delegated connector + per-user OAuth flow + disable-revokes | The named customer scenario; rides on phase-2 rails |
| **5** | Lane B: connector sync jobs → RAG ingestion, `ConnectorBinding` scoped folders (Harvey-style), ACL snapshot sync | Needs 1+2+4; delivers "their data in Microsoft 365, searchable in chat, permission-trimmed" |
